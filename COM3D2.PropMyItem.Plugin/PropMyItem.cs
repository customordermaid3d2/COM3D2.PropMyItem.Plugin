...
namespace COM3D2.PropMyItem.Plugin
{
    ...
    public class PropMyItem : PluginBase
    {
        ...
        private System.Collections.IEnumerator CheckMenuDatabase()
        {
            ...
            isRuning = true;
            StartCoroutine(LoadMenuFiles());

            yield return new WaitWhile(() => isRuning);
        }
            ...
            try
            {
                ...
                else
                {
                    if (this._isVisible && (!this._isStartUpLoadead || this._isForcedInit))
                    {
                        ...
                        {
                            Console.WriteLine("PropMyItem: LoadMenuFiles isRuning");
                        }
        ...
        static bool isRuningSub = false;

        static List<MenuInfo> variationMenuList;
        List<SMenuInfo> saveItems;

        //public void LoadMenuFiles()
        public IEnumerator LoadMenuFiles()
        {
            isRuning = true;
            Stopwatch stopwatch = new Stopwatch(); //객체 선언
            stopwatch.Start(); // 시간측정 시작

            Console.Write("LoadMenuFiles st" + string.Format("{0:0.000} ", stopwatch.Elapsed.ToString()));

            saveItems = new List<SMenuInfo>();

            Dictionary<string, MenuInfo> dictionary = new Dictionary<string, MenuInfo>();
            try
            {
                if (!this._isForcedInit)
                {
            ...
            variationMenuList = new List<MenuInfo>();

            Console.Write("LoadMenuFiles md1" + string.Format("{0:0.000} ", stopwatch.Elapsed.ToString()));

            isRuningSub = true;
            StartCoroutine(this.GetMainMenuFiles( dictionary, dictionary2, dictionary3)); //86
            yield return new WaitWhile(() => isRuningSub);

            Console.Write("LoadMenuFiles md2" + string.Format("{0:0.000} ", stopwatch.Elapsed.ToString()));

            isRuningSub = true;
            StartCoroutine(this.GetModFiles( dictionary, dictionary2, dictionary3)); //10
            yield return new WaitWhile(() => isRuningSub);

            Console.Write("LoadMenuFiles md3" + string.Format("{0:0.000} ", stopwatch.Elapsed.ToString()));

            isRuningSub = true;
            StartCoroutine(this.SetVariationMenu(dictionary2, dictionary3));
            yield return new WaitWhile(() => isRuningSub);
            ...
            try
            {
                ...
                this.setColorSet();
                Config.Instance.MenuItems = saveItems;
            ...
            Console.Write("LoadMenuFiles ed" + string.Format("{0:0.000} ", stopwatch.Elapsed.ToString()));
            isRuning = false;
        }
        ...
        private IEnumerator GetMainMenuFiles( Dictionary<string, MenuInfo> loadItems, Dictionary<string, string> favDic, Dictionary<string, string> colorLockDic)
        {
            isRuningSub = true;
            Stopwatch stopwatch = new Stopwatch(); //객체 선언
            stopwatch.Start(); // 시간측정 시작
            ...
            Console.Write("GetMainMenuFiles md0 " + string.Format("{0:0.000} , {1}", stopwatch.Elapsed.ToString(), files.Length));
            ...
            int s = database.GetDataSize();

            Console.Write("GetMainMenuFiles md1 " + string.Format("{0:0.000} , {1}", stopwatch.Elapsed.ToString(),s ));
            ...
            string[] l = GameUty.ModOnlysMenuFiles;

            Console.Write("GetMainMenuFiles md2 " + string.Format("{0:0.000} , {1}", stopwatch.Elapsed.ToString(), l.Length));

            foreach (string menuFile in l)
            {
                ParseMainMenuFile(menuFile, list, ref variationMenuList, loadItems, dictionary, favDic, colorLockDic, ref saveItems);
                yield return null;
            }

            Console.Write("GetMainMenuFiles md3 " + string.Format("{0:0.000} ", stopwatch.Elapsed.ToString()));
        ...
        private IEnumerator GetModFiles( Dictionary<string, MenuInfo> loadItems, Dictionary<string, string> favDic, Dictionary<string, string> colorLockDic)
        {
            isRuningSub = true;
            ...
            Console.Write("GetModFiles " + string.Format("{0:0.000} , {1}", stopwatch.Elapsed.ToString(), files.Length));
            ...
            Console.Write("GetModFiles " + string.Format("{0:0.000} , {1}", stopwatch.Elapsed.ToString(), list.Count));
            ...
            Console.Write("GetModFiles " + string.Format("{0:0.000} ", stopwatch.Elapsed.ToString()));

            isRuningSub = false;
        }

        // Token: 0x06000046 RID: 70 RVA: 0x000076B4 File Offset: 0x000058B4
        private IEnumerator SetVariationMenu(Dictionary<string, string> favDic, Dictionary<string, string> colorLockDic)
        {
            isRuningSub = true;
            Stopwatch stopwatch = new Stopwatch(); //객체 선언
            stopwatch.Start(); // 시간측정 시작

            Console.Write("SetVariationMenu st" + string.Format("{0:0.000} , {1}", stopwatch.Elapsed.ToString(), variationMenuList.Count));
            ...
            Console.Write("SetVariationMenu ed" + string.Format("{0:0.000}", stopwatch.Elapsed.ToString()));
